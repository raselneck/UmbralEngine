...
#define RESOURCE(NAME, FIELD, CNAME) \
  case Limit::NAME:                  \
    limits_.FIELD = value;           \
    break;
#include "shaderc_util/resources.inc"
#undef RESOURCE
  }
}
...
std::vector<uint32_t> ConvertStringToVector(const std::string& str) {
  ...
  size_t vector_length = (num_bytes_str + sizeof(uint32_t) - 1) / sizeof(uint32_t);
  std::vector<uint32_t> result_vec(vector_length, 0);
#if WIN32
  ::strncpy_s(reinterpret_cast<char*>(result_vec.data()), result_vec.size() / sizeof(uint32_t), str.c_str(), str.size());
#else
  std::strncpy(reinterpret_cast<char*>(result_vec.data()), str.c_str(), str.size());
#endif
  return result_vec;
}
