...
#ifdef _MSC_VER
#pragma warning (push)
#pragma warning (disable: 4127)     // conditional expression is constant (for IM_ASSERT(0 && ...))
...
namespace
{
    ...
    static const char* GetFreeTypeErrorString(const FT_Error error)
    {
        switch (error)
        {
        /* generic errors */
        ...
        case FT_Err_Invalid_SVG_Document: return "invalid SVG document";
        ...
        case FT_Err_DEF_In_Glyf_Bytecode: return "found FDEF or IDEF opcode in glyf bytecode";
        case FT_Err_Missing_Bitmap: return "missing bitmap in strike";
        case FT_Err_Missing_SVG_Hooks: return "SVG hooks have not been set";
        ...
        case FT_Err_Glyph_Too_Big: return "glyph too big for hinting";
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, unsigned int extra_font_builder_flags)
    {
        ...
        if (error != 0)
        {
            IMGUI_DEBUG_PRINTF("Failed to load font from memory (code=%d). Reason: %s\n", error, GetFreeTypeErrorString(error));
            return false;
        }
        error = FT_Select_Charmap(Face, FT_ENCODING_UNICODE);
        if (error != 0)
        {
            IMGUI_DEBUG_PRINTF("Failed to select unicode charmap in font (code=%d). Reason: %s\n", error, GetFreeTypeErrorString(error));
            return false;
        }
...
static bool ImFontAtlasBuildWithFreeType(ImFontAtlas* atlas)
{
    ...
    if (error != 0)
    {
        IMGUI_DEBUG_PRINTF("Failed to initialize FreeType (code=%d). Reason: %s\n", error, GetFreeTypeErrorString(error));
        return false;
    }
